...
then
    ...
    grep -rl "$value" . --exclude-dir=ArbitriumCLI --exclude=*setAPI_FQDN.sh | xargs sed -i "s/$value/$1/g"
    grep -rl "$value_IP" . --exclude-dir=ArbitriumCLI --exclude=*setAPI_FQDN.sh | xargs sed -i "s/$value_IP/$resolvedDNS/g"
    echo "[!] Done"
fi
